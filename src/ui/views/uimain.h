...
signals:
    void musicPlayed(const QModelIndex &index, bool autoPlay); // 播放歌曲
    void musicPlayedOrPaused(); // 播放与暂停
    void musicSkipped(bool isNext); // 上/下一首音乐
    void modeChanged(); // 播放模式更改
    ...
    void playlistCoverUpdated(const QString &path, int playlist_id); // 歌单封面更新

public slots:
    void doubleClickPlay(const QModelIndex &index, bool autoPlay); // 音乐列表双击事件
    void changePlayMode(PlayMode mode); // 播放模式更改
    void skipMusic(bool isNext); // 上/下一首按钮点击事件
    void addContextMenuPlaylist(const PlayListInfo &info); // 为右键菜单添加歌单
    void removeContextMenuPlaylist(int playlist_id); // 为右键菜单移除歌单
...
private:
    ...
    void addMusicToPlaylist(QAction *action); // 添加音乐到歌单
