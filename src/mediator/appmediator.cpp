...
void AppMediator::playConnect()
{
    ...
    // 双击播放
    connect(m_uiMain, &UIMain::musicPlayed, this, &AppMediator::playMusic);

    // 手动控制播放/停止
    connect(m_uiMain, &UIMain::musicPlayedOrPaused, m_controller, &PlayerController::playOrPause);

    // 下/上一首
    connect(m_uiMain, &UIMain::musicSkipped, this, &AppMediator::skipMusic);
    ...
    // 播放模式切换
    connect(m_uiMain, &UIMain::modeChanged, m_listManager, [this](){
        PlayMode mode = static_cast<PlayMode>((m_listManager->mode() + 1) % PlayMode::Count);
        m_listManager->setMode(mode);
    });
    connect(m_listManager, &PlayListManager::modeChanged, m_uiMain, &UIMain::changePlayMode);
...
void AppMediator::playlistConnect()
{
    ...
    // 侧边栏创建歌单，更新主界面歌单列表
    connect(m_uiSideBar, &UISideBar::playlistCreated, m_uiMain, &UIMain::addContextMenuPlaylist);

    // 更新侧边栏歌单封面
    connect(m_uiMain, &UIMain::playlistCoverUpdated, m_uiSideBar, &UISideBar::setPlaylistCover);
...
void AppMediator::collectSong(bool isCollect, const QModelIndex &index)
{
    ...
    bool dbOk = false;
    if(isCollect) {
        dbOk = DbManager::getInstance().collectSong(idx);
    }
    else {
        dbOk = DbManager::getInstance().disCollectSong(idx);
    }
...
void AppMediator::playMusic(const QModelIndex &index, bool autoPlay)
{
    ...
    if(proxyModel == nullptr){  // index属于源模型
        sourceIndex = index;
        isSource = true;
    }
    else {  // index属于代理模型
        sourceIndex = proxyModel->mapToSource(index);
        m_listManager->setRows(m_uiMain->getPlaylistRows(proxyModel));
        isSource = false;
    }

    m_listManager->setIsSource(isSource);
    m_controller->playMusic(sourceIndex, autoPlay);
    m_listManager->setCurrentRow(sourceIndex.row());
